...
res_oc = res["Listado"][0]

def retrieved_oc_values():
    for oc in res_oc:
        r_oc_number = res_oc['Codigo']
        r_oc_status = res_oc['CodigoEstado']
        r_oc_name = res_oc['Nombre']
        sql = 'INSERT INTO Purchase_orders_orders (number, status_code, name)'
        val = (r_oc_number,r_oc_status,r_oc_name)
        cur.execute(sql,val)
